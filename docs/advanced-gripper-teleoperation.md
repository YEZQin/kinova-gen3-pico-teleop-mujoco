...
The hardware-observed baseline uses no `InvertTranslation` and `LinearGain 1.0`. Optional inversion or a higher gain must be treated as a separate tuning trial. `MaxLinearSpeed` remains the unchanged hard velocity cap. While Grip stays active, an unchanged gripper target is reasserted every 0.25 seconds. Status output reports `gripper_target`; evidence records `gripper_commanded` only after an actual Kortex gripper RPC succeeds.
...
实机观察到的基线不使用 `InvertTranslation`，并采用 `LinearGain 1.0`。可选反向或更高增益必须作为独立调参试验。`MaxLinearSpeed` 仍是未改变的速度硬上限。Grip 持续有效时，未改变的夹爪目标每 0.25 秒重申一次。终端状态显示 `gripper_target`；只有 Kortex 夹爪 RPC 实际成功后，evidence 才记录 `gripper_commanded`。
